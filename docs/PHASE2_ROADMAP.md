...
**접근 제어 설계**: `ADMIN_EMAILS`(서버 전용 환경변수, 쉼표 구분)에 등록된
이메일만 접근 가능. 클라이언트에는 "누가 관리자인지" 전혀 노출하지 않고,
모든 실제 권한 검사는 서버(API 라우트)에서 매 요청마다 수행한다 —
관리자 페이지 자체의 진입 가드는 UX 용도일 뿐, 진짜 방어선은
`lib/adminAuth.ts`의 `verifyAdmin()`이다.

**구현**:
- `lib/adminAuth.ts` — 호출자의 로그인 토큰을 검증하고, 이메일이
  `ADMIN_EMAILS` 목록에 있는지 확인. 목록이 비어있거나 토큰이 없으면
  무조건 거부
- `/api/admin/overview`, `/api/admin/moderation-reports`,
  `/api/admin/purchase-orders` — 전부 `verifyAdmin()` 통과해야만
  service_role로 데이터 조회
- `/admin` — 가입자 수·매출 등 운영 현황 요약, **콘텐츠 감수 신고
  내역**(지금까지 DB에만 쌓이고 볼 방법이 없었던 데이터를 처음으로
  화면에서 확인 가능하게 함), 결제 내역

**검증**: Authorization 헤더 없음, `ADMIN_EMAILS` 미설정 두 경우 모두
안전하게 거부되는 것을 확인했다.

**필요 작업**: EC2 `.env.local`에 `ADMIN_EMAILS=본인이메일` 추가 후
재배포.

---

## 26. 관리자 페이지 실운영 기능 확장

첫 버전(개요·신고내역·결제내역, 읽기 전용)에서, 실제 운영하면서 필요할
기능들을 분석해 추가했다.

**추가된 기능**:
- **회원 조회 + 캐시 수동 조정**: 닉네임으로 검색 → 보유 캐시, 최근
  거래내역 확인 → 금액/사유 입력해 즉시 지급·차감. "결제했는데 캐시가
  안 들어왔어요" 같은 CS 문의 대응에 필수적인 기능이라 가장 먼저 추가.
  새 SQL 함수 `admin_adjust_cash()`(SECURITY DEFINER, service_role만
  실행 가능하도록 일반 사용자 EXECUTE 권한 차단)로 잔액을 조정하고
  `wallet_transactions`에 사유와 함께 로그를 남긴다
- **연동 설정 상태**: 쿠팡파트너스·토스페이먼츠(+테스트/라이브 구분)·GAM
  광고·카카오 공유 등이 실제로 설정됐는지 한눈에 확인 — 매번 SSH로
  `.env.local`을 열어보지 않아도 됨. **실제 키 값은 절대 반환하지 않고
  설정 여부(boolean)만** 응답한다
- **상품별 판매 현황**: `wallet_transactions`를 `product_code` 기준으로
  집계해, 어떤 유료 콘텐츠(정밀 리포트/심층 궁합/대운세운/오늘의 카드)가
  실제로 팔리는지 확인
...
## 33. 마일스톤 미션 개선 — 악용 방지 + 성장 보너스 2종

**1. 친구초대 악용 방지**: 구글/카카오 계정을 무의미하게 여러 개 만들어
서로 초대·수락만 해서 보상을 타가는 걸 막았다.
- 같은 상대와 중복 친구관계가 생겨도 `count distinct`로 1명만 인정
- 상대방이 **실제로 사주 분석을 완료한 계정**(`user_profiles.lpt_type_id`
  존재)일 때만 인정 — 빈 껍데기 계정으로는 카운트 안 됨. 가짜 계정을
  만들어도 매번 생년월일 입력+설문까지 실제로 거쳐야 해서 대량 생성
  비용이 커진다
- 상대방 계정이 **생성된 지 최소 하루는 지나야** 인정 — 즉석 대량 생성
  후 바로 수락하는 방식을 늦춘다

**2. 성장 보너스 2종 추가**: 친구초대 외에도 앱 안에서의 성장 자체로
보상받을 수 있게 했다.
- **레벨 달성**: Lv.3(300캐시)/5(700)/7(1500)/10 만렙(3500) — SQL의
  레벨 계산 로직이 앱의 `LEVEL_THRESHOLDS`와 정확히 일치하는지
  0~3000 XP 구간 전수 대조 검증했다
- **뱃지 수집**: 2개(300)/4개(800)/8개 전체(2500)

**필요 작업**: `supabase/migrations/019_milestone_improvements.sql`을
SQL Editor에서 실행 (018 이후 순서로).

---

## 34. 두 종류 캐시 분리 — 무료 지급이 AI 원가 상품 수익을 잠식하는 문제 해결

**배경**: 30년 경력 비즈니스 전문가 관점 분석 요청에 따라 검토한 결과,
한 사용자가 웰컴캐시+마일스톤 보상을 전부 채우면 최대 약 20,300캐시를
무료로 받을 수 있는데, 이게 AI 호출 원가가 있는 상품(정밀리포트 등,
120배 마진으로 설계했던 상품들)에도 무제한으로 쓰일 수 있는 구조적
결함이 있었다. 원가는 실제로 나가는데 매출은 0원이 되는 상황이
반복될 수 있어, 게임업계의 "소프트캐시/하드캐시" 패턴으로 분리했다.

**설계 원칙**:
- `wallets.bonus_balance`(신규) — 무료 지급(웰컴캐시·마일스톤·광고
  리워드)이 쌓이는 곳
- `wallets.cash_balance`(기존) — 실제 결제(토스페이먼츠)로만 채워짐
- **`product_prices.ad_unlockable = false`인 상품(AI 원가 있음:
  정밀리포트·대운세운·이번달운세·심층궁합)은 `cash_balance`만 쓸 수
  있다 — `bonus_balance`가 아무리 많아도 절대 안 씀**
- `ad_unlockable = true`인 상품(오늘의 카드, 원가 거의 없음)은
  `bonus_balance`를 먼저 쓰고 모자란 만큼만 `cash_balance`에서 채움
- 관리자 수동 조정(CS 대응)은 `cash_balance`를 조정(실제 결제 문제
  보상이므로 변경 없음)

**수정한 함수 5개**: `grant_ad_cash_reward`, `unlock_daily_content`,
`purchase_product`, `handle_new_user_wallet`, `claim_milestone_reward`
— 전부 무료 지급분은 bonus_balance로, 결제 로직은 위 원칙에 따라
분기하도록 재작성.

**UI**: 대시보드·계정 화면 둘 다 "실제캐시 X · 보너스캐시 Y" 형태로
구분 표시. 보너스캐시가 있으면 "AI 분석 콘텐츠는 실제캐시로만 결제"
안내 문구 노출. 관리자 회원조회 화면에도 보너스캐시 별도 표시(조정은
여전히 실제캐시만 대상).

**검증**: 4가지 핵심 시나리오(보너스만 있고 AI상품 구매 시도/실제캐시
있을 때 정상구매/원가없는상품 총합부족/원가없는상품 보너스+실제 혼합
차감)를 로직으로 직접 시뮬레이션해 전부 의도대로 동작하는 것을
확인했다.

**필요 작업**: `supabase/migrations/020_bonus_cash_split.sql`을
SQL Editor에서 실행.

---

## 35. "캐시" → "보석"/"별조각" 전면 교체 + 가운뎃점 정리

**배경**: "캐시"라는 표현이 법적으로 민감할 수 있고(선불전자지급수단 등
규제 용어와 혼동 소지), 실제캐시/보너스캐시라는 명칭도 사용자 입장에서
헷갈린다는 지적이 있었다.

**새 이름**: 보석(구매로 충전, 모든 상품 결제 가능) / 별조각(웰컴·
마일스톤·광고로 무료 지급, AI 원가 있는 상품에는 못 씀).

**작업 범위**:
- 화면에 보이는 모든 JSX 텍스트에서 "캐시" 완전히 제거(전수 검색으로
  확인) — 지갑/충전/미션/관리자/공유링크 등 관련 화면 전부
- **DB 함수가 거래내역에 남기는 description 문구와 raise exception
  에러 메시지**도 함께 정리(`charge_cash_from_pg`, `grant_ad_cash_reward`,
  `unlock_daily_content`, `purchase_product`, `handle_new_user_wallet`,
  `claim_milestone_reward` 6개 함수 전부 재정의) — 화면 텍스트만
  고치고 이걸 놓치면 "최근 거래 내역"에 여전히 "캐시"가 남는 문제가
  있어 함께 처리했다
- 이미 쌓인 기존 거래내역(테스트 데이터)의 description도 일괄
  `update`로 소급 정리
- 관련 화면의 가운뎃점(·)도 괄호나 자연스러운 문장으로 교체(공유 링크
  목록, 관리자 화면, 쿠팡 로켓배송 표시 등). 사주 결과 표시(십성 설명,
  MBTI 축 설명)의 가운뎃점은 이전에 이미 "자연스러운 한국어 압축
  표현"으로 검토를 거쳐 유지하기로 한 부분이라 이번 범위에서 제외했다

**검증**: JSX 렌더링 텍스트(주석 제외) 전수 검색으로 "캐시" 잔존
없음을 확인했다.
...
## 37. 수익 모델 2차 검증 + 결제 취소/환불 대응 구현

**검증 1 — 지난번 구조적 결함(무료캐시가 AI원가 상품 잠식)이 실제로
막혔는지 재확인**: AI 원가 있는 상품 4개(정밀리포트·심층궁합·대운세운·
이번달운세) 전부 `ad_unlockable=false`로 정확히 설정되어 있어 보석
전용으로 막혀있음을 코드로 직접 확인. "오늘의 카드"도 여전히 정적
큐레이션 데이터라 원가 없음 확인. **문제없음.**

**검증 2 — "이번 달 운세" AI 원가 추정**: 대운세운과 프롬프트 길이를
직접 비교해, 추정 원가 15~18원 대비 판매가 700원 — 마진율 97%대
(약 40~47배). 다른 상품보다는 낮지만 여전히 건전한 수준.

**검증 3 — 새로 발견한 실제 위험: 결제 취소/환불(차지백) 미대응**:
지금까지는 "결제 대기 중" 주문의 취소만 처리했고, **이미 완료된
결제가 나중에 취소되는 경우(카드사 차지백 등)는 아무 대응이 없었다.**
사용자가 결제 후 콘텐츠를 소비한 뒤 결제를 취소하면, 매출은 사라지는데
소비된 콘텐츠(AI 원가)는 회수가 안 되는 구조였다.

**구현한 대응**:
- `payment_refund_events` 테이블 + `process_payment_refund()` 함수 —
  이미 완료된 주문이 취소되면, **남아있는 보석만큼만 회수**(이미
  소비했으면 회수 불가능한 만큼은 "미회수액"으로 별도 기록)
- 웹훅이 이제 pending 주문뿐 아니라 completed 주문의 취소도 감지해
  자동으로 이 회수 처리를 트리거
- 관리자 페이지에 "결제 취소·환불 내역" 섹션 추가 — 미회수액이 있으면
  빨간 테두리로 강조 표시, "확인함" 처리 가능. 개요 통계에 "미확인
  환불" 건수도 노출

**검증**: 자산 회수 계산 로직(전액 소비/일부 소비/미소비/다른 충전과
섞인 경우) 4가지 핵심 시나리오를 직접 시뮬레이션해 전부 의도대로
정확히 동작하는 것을 확인했다.

**필요 작업**: `supabase/migrations/022_payment_refund_handling.sql`을
SQL Editor에서 실행.
