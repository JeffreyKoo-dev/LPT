...
## 4. schema.sql 함수 중복 정리 (낮은 우선순위)

`create or replace function`이 여러 마이그레이션에 걸쳐 반복 추가되면서
`schema.sql`(새 프로젝트 통합 세팅용 파일) 안에 같은 함수의 예전 버전이
남아있을 수 있다(마지막 정의가 실행되므로 기능적으로는 문제없지만
파일이 지저분함). 시간 날 때 한 번 정리 권장.
