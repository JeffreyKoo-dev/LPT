...
- 사주 계산은 `src/lib/saju-engine/`에 내재화한 만세력 엔진(원본: [ssaju](https://github.com/golbin/ssaju), MIT License — KASI 음력 데이터 + 절입시각 기반)으로 정밀 계산합니다. 대한민국 표준시 변경 이력(1908~1911, 1954~1961년 UTC+8:30 기간)과 진태양시(선택 옵션) 보정도 반영합니다 (`lib/saju.ts` 상단 주석 참고). 외부 npm 패키지가 아니라 프로젝트 코드로 내재화되어 있어 직접 수정할 수 있습니다 (`src/lib/saju-engine/README.md` 참고)
...
- 십성 계산은 천간·지지를 모두 반영합니다 (지지는 지장간 정기 기준). 지장간 여기·중기까지 상세히 펼쳐 보여주지는 않지만, 엔진 데이터에는 포함되어 있어 필요 시 UI만 추가하면 확장 가능합니다
- 서머타임(일광절약시간) 보정은 `saju-engine`에 정확한 일자로 반영된 1960/1987/1988년 3개 기간만 적용됩니다. 1948~1951, 1955~1959년에도 실제 시행됐지만 신뢰할 수 있는 일 단위 기록을 확인하지 못해 반영하지 않았습니다 (`src/lib/saju-engine/README.md` 참고) — 이 기간 출생자는 시주가 최대 1시간 어긋날 수 있습니다

## 배포 후 예전 데이터로 인한 크래시 방지

이 서비스는 LocalStorage에 데이터를 저장하므로, 저장된 데이터의 "형태(구조)"가
바뀌는 업데이트를 배포하면 사용자 기기에 남아있는 예전 형태의 데이터가 새 코드와
맞지 않아 화면이 깨질 수 있다. 두 가지 안전장치를 두었다.

1. **`app/error.tsx`, `app/global-error.tsx`** — 렌더링 중 예외가 발생하면 흰
   화면 대신 복구 화면이 뜬다. "다시 시도" · "홈으로" · "내 데이터 초기화하고
   새로 시작" 세 가지 선택지를 제공한다.
2. **스키마 버전 검증** — `AnalysisReport`에 `schemaVersion`을 포함시켜
   (`types/report.ts`의 `ANALYSIS_SCHEMA_VERSION`), 구조가 바뀌는 배포를 할
   때마다 이 값을 올린다. `loadAnalysisReport()`가 버전이 다른 데이터를 만나면
   자동으로 무효 처리하고 지운다. `useGrowthSession`, `/result` 페이지는 이때
   기본 정보와 설문 응답이 남아있으면 사용자가 설문을 다시 할 필요 없이 조용히
   재계산한다.

**앞으로 저장 데이터의 구조를 바꾸는 변경(필드 추가가 아니라 기존 필드의 타입이나
형태를 바꾸는 경우)을 할 때는 `ANALYSIS_SCHEMA_VERSION`을 올려야 한다.** 다른
저장 데이터(`GrowthProfile` 등)도 구조를 바꾸게 되면 같은 패턴(버전 필드 추가 +
로드 시 검증)을 적용하는 것을 권장한다.
